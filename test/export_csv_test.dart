...
void main() {
  test('Export 50 activities to CSV file', () async {
    final repo = FirestoreActivityRepository();
    final activities = await repo.getAllActivities();

    final sb = StringBuffer();
    // UTF-8 BOM Header for Excel / Google Spreadsheet compatibility
    sb.write('\uFEFF');
    ...
    for (final a in activities) {
      final locations = a.locationTypes.map((e) => e.name).join(';');
      final tools = a.toolsRequired.join(';');
      final abilities = a.trainableAbilities.map((e) => e.name).join(';');
      final safetyTips = a.safetyTips.join(';');
      final steps = a.steps.join(';');
      final praiseTips = a.parentPraiseTips.join(';');
      final seasons = a.seasons.map((e) => e.name).join(';');
